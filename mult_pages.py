	...
	async def exec(self):
		...
		while True:
			if(self.window.flags[0] and not page_num == 0):
				...
				self.window.frames[NavigationPage].paint()#absolutely disgusting
...
class DeveloperMode(tk.Frame):
	def __init__(self, parent, controller: tkinterApp):
		...
		'''ck_1_value = tk.BooleanVar(value=True)
		ck_1 = ttk.Checkbutton(text="Beacon #1", variable=ck_1_value)
		ck_2_value = tk.BooleanVar(value=True)
		ck_2 = ttk.Checkbutton(text="Beacon #2", variable=ck_2_value)
		ck_2_value = tk.BooleanVar(value=True)
		ck_2 = ttk.Checkbutton(text="Beacon #2", variable=ck_2_value)'''
		...
		for beacons_index in range(len(self.controller.beacons_list)):
			...
			ck = ttk.Checkbutton(master=self,text=beacon_id, variable=self.controller.ck_bool[beacons_index])
